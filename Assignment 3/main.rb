		...
		feature.locations.each_with_index do |loc, locindex|
			...
			if strand == 1
				...
				matches.each_with_index do |match, matchindex|
					...
					$gen_mtch_p_ini_coord_arr << rep_coord_ini
				end
			end
			# minus_strand
			if strand == -1			
				...
				matches.each_with_index do |match, matchindex|
					...
					$gen_mtch_m_ini_coord_arr << rep_coord_ini
				end
			end
		end
	end
...
if $task_4b_arr.length > 0
	...
	header += "#.> Candela Migoyo Bettoni and Alvaro Mellado Ibañez - #{DateTime.now.strftime("%d.%m.%Y")}\n"
